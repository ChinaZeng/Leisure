 ...
 * 描述:
 */
public class GalleryClassAdapter extends BaseRecyAdapter<GalleryClassModel> {

    private List<Integer> mHeights = new ArrayList<>();
    private LayoutInflater inflater;
    private int ScreenHeight;

    public GalleryClassAdapter(Activity context) {
        super(context);
        inflater = LayoutInflater.from(context);
        ScreenHeight = TDevice.getDisplayMetrics(context).heightPixels;
    }

    private void addHeights(List<GalleryClassModel> list) {
    ...
    @Override
    public void addItems(List<GalleryClassModel> list) {
        mData.addAll(list);
        addHeights(list);
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof GalleryViewHolder) {
            ...
            final GalleryClassModel model = mData.get(position);
            viewHolder.item_gallery_title.setText(model.getTitle());
            viewHolder.item_gallery_img.setImageResource(model.getSrcId());
            ...
            viewHolder.item_gallery_root.setOnClickListener(new View.OnClickListener() {
                ...
                public void onClick(View v) {
                    UI.showGalleryActivity(context, model.getTitle(), model.getId());
                }
            });
        }
    }
